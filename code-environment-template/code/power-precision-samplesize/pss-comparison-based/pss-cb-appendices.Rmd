...
We want to estimate $\delta = \mu_{Y(1)}$. The empirical average of the pre-post differences is $\hat\delta = \hat\mu_{Y(1)}$.

We have:
  \[
    \begin{aligned}
    Var \big( \hat\delta \big)
    & = Var \left( \hat\mu_{Y(1)} \right) \\
    & = Var \left(
      \frac{1}{n_1} \sum_{i=1}^{n_1} Y_{1,i}
      \right) \\
...
Suppose $V$ is log-normally distributed. Then equivalently, $Y = \ln( V )$ is normally distributed as $N ( \mu, \sigma )$ with mean $\mu$ and standard deviation $\sigma$. We have the mean and median of $V$, $\mu_V = \exp \big( \mu + \frac{1}{2} \sigma^2 \big)$ and $\text{med}_V = \exp ( \mu )$, respectively. The median of $V$ is equal to its geometric mean.

The quantile function of $V$ for quantile or percentile value $p \in (0, 1)$ is $q_V ( p ) = \exp \big( \mu + \sigma \Phi^{-1} (p) \big)$, where $\Phi(z)$ represents the standard normal $N (0, 1)$ cumulative distribution function of $Z$ at $z$. Equivalently:
