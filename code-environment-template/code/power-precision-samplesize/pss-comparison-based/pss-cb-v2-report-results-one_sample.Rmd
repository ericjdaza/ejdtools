...
We want to statistically discern or detect a hypothesized true average non-zero pre-post difference (i.e., change over time, effect size) of size $\delta$ or larger in continuous outcomes $Y$. We will assume these outcomes vary with the same standard deviation (SD) at either time point. We will also assume these outcomes are fairly normally distributed, allowing us to rely on the paired-samples t-test in our calculations carried out via the R command `stats::power.t.test()`.
