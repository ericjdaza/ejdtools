---
title: "Project: [insert project name or codename]: Power and Sample Size Calculations for One Group"
...
tbl_sims_final_ntests <- readr::read_csv(paste0(output_path, "tbl_sims_final_ntests", tbl_pss_parameters$num_of_tests, ".csv")) %>% dplyr::filter(type_of_test == "one.sample")
tbl_sims_fancy_ntests <- readr::read_csv(paste0(output_path, "tbl_sims_fancy_ntests", tbl_pss_parameters$num_of_tests, ".csv")) %>% dplyr::filter(`Type of Test` == "one-sample") %>% dplyr::select(-`Type of Test`)
tbl_sims_sentences_ntests <- readr::read_csv(paste0(output_path, "tbl_sims_sentences_ntests", tbl_pss_parameters$num_of_tests, ".csv")) %>% dplyr::filter(type_of_test == "one.sample") %>% dplyr::select(-type_of_test)
...
We want to statistically discern or detect a hypothesized true average difference from zero (i.e., association or effect size) of size $\delta$ or larger in continuous outcomes $Y$ in one treatment group. We will assume these outcomes vary with the same standard deviation (SD) $\sigma_Y$ at any time point. These two quantities are often combined as Cohen's $d$, defined as $d = \delta \big/ \sigma_Y$. We will also assume these outcomes are fairly normally distributed, allowing us to rely on Cohen's $d$ and the one-sample t-test in our calculations carried out via the R command `pwr::pwr.t.test()`.
...
The most conservative sample size will be the largest one we anticipate needing. To calculate this, we need to use the smallest hypothesized true average difference among the `r tbl_pss_parameters$num_of_tests` a priori research hypotheses, along with the outcome with the largest SD. Based on our literature review and scientific judgment, we will assume that the smallest true average difference is `r tbl_pss_parameters$delta_primary` units, and that the largest SD of pre- and post-treament outcomes is `r tbl_pss_parameters$sd_primary`. The same sample size results will apply if the smallest true average difference is `r tbl_pss_parameters$delta_secondary %>% round(2)` units, and the largest SD of pre- and post-treament outcomes is `r tbl_pss_parameters$sd_secondary`.
...
Here is an example of two extremes of required sample sizes for one-sample tests:
...
- For any particular tolerable overall FPR, the number of participants required increases with higher desired statistical power.
...
Let $\sigma^2$ represent the variance of raw outcomes at either pre- or post-treatment. Then the SD of the pre-post difference $Y$ is $\sigma_Y = \sqrt{2 \sigma^2 (1 - ICC)}$ (see **Appendix: Repeated Measures**). From this, we see that the higher the positive ICC, the lower the variance of the pre-post difference. This lowers the sample size we'd need if $ICC > 0.5$. Specifically, if $ICC = 0.5$, then the variance we'd specify in our PSS calculations is simply $\sigma^2$, the variance of outcomes at either pre or post period.
...
We want to estimate $\delta = \mu_1$. The empirical average of the mean outcome is $\hat\delta = \hat\mu_1$.

We have:
\[
\begin{aligned}
Var \left( \hat\delta \right)
& = Var \left( \hat\mu_1 \right) \\
& = Var \left(
  \frac{1}{n_1} \sum_{i=1}^{n_1} Y_{1,i}
  \right) \\
& = \frac{1}{n_1} Var \left( Y_{1,i} \right) \\
& = \frac{1}{n_1} Var(Y)
\end{aligned}
\]
...
\end{aligned}
\]
If $Var \left( Y_{post} \right) = Var \left( Y_{pre} \right) \equiv \sigma^2$, this simplifies to $Var(Y) = 2 \sigma^2 - 2 ICC \sigma^2 = 2 \sigma^2 (1 - ICC) = \sigma_Y$.
